...
import { User, Wallet, Transaction } from '@/interfaces';
...
export default function WalletsPage() {
  ...
  const allTransactions = users?.reduce((acc: Transaction[], user) => {
  ...
  // حساب الإحصائيات
  const stats = {
    totalBalance: users?.reduce((sum, user) => sum + (user.wallet?.balance || 0), 0) || 0,
    totalTransactions: filteredTransactions?.length || 0,
    totalDeposits: filteredTransactions?.filter(t => t.type === 'deposit').reduce((sum, t) => sum + t.amount, 0) || 0,
    totalWithdrawals: filteredTransactions?.filter(t => t.type === 'withdrawal').reduce((sum, t) => sum + t.amount, 0) || 0,
  };
      ...
      <Grid container spacing={3} mb={4}>
        <Grid item xs={12} md={3}>
          <Paper sx={{ p: 3, display: 'flex', alignItems: 'center', gap: 2 }}>
            ...
            <Box>
              <Typography variant="h6">{stats.totalBalance}</Typography>
        ...
        <Grid item xs={12} md={3}>
          <Paper sx={{ p: 3, display: 'flex', alignItems: 'center', gap: 2 }}>
            ...
            <Box>
              <Typography variant="h6">{stats.totalWithdrawals}</Typography>
              <Typography variant="body2" color="text.secondary">
                إجمالي السحوبات
              </Typography>
            </Box>
